...
`.trim()
    ...
    */
    ...
    conn.sendButtonDoc = async (jid, content, footerText, button1, id1, quoted, options) => {
      ...
      const buttonMessage = {
        document: { url: "https://wa.me/+6282161429908" },
...
/*exports.smsg = (conn, m, hasParent) => {
    ...
    m.reply = async (text, chatId, options) => {
        ...
        conn.reply(chatId ? chatId : m.chat, text, m, { contextInfo: { mentionedJid: conn.parseMention(text), externalAdReply: { title: 'VynaaMD🦄', body: wm, sourceUrl: 'https://youtube.com/@VynaaChan', thumbnail: data }}, options })
    }
    m.name = m.pushName || conn.getName(m.sender)
